...
pub async fn run() {
    tracing::info!("Starting Secure Receiver node...");
    ...
    tracing::info!("Waiting for sender handshake...");
    let n = match stream.read(&mut buf).await {
        Ok(n) if n > 0 => n,
        _ => {
            tracing::error!("Failed to receive handshake.");
            return;
        }
    };
    ...
    let recv_packet = match Packet::from_bytes(&buf[0..n]) {
        Ok(p) => p,
        Err(_) => {
            tracing::error!("Corrupted handshake.");
            return;
        }
    };
    ...
    let session_crypto = match recv_packet.payload {
        PayloadType::Handshake(bytes) => {
            ...
            if bytes.len() == 32 {
                ...
                tracing::error!("Invalid key size.");
                return;
            }
        }
        _ => {
            tracing::error!("Expected handshake.");
            return;
        }
    };
    
    // Send own Handshake
    tracing::info!("Sending handshake back...");
    let handshake_packet = Packet::new(0, "Receiver".to_string(), PayloadType::Handshake(my_pub_key.as_bytes().to_vec()));
    ...
    println!("--- Secure E2E Chat (Receiver) ---");
                        ...
                        match packet.payload {
                            PayloadType::TextChat(encrypted) => {
                                if let Ok(decrypted) = session_crypto_clone.decrypt(&encrypted) {
                                    ...
                                    print!("Receiver > "); io::stdout().flush().unwrap();
                                }
                                // Send Ack
                                let ack = Packet::new(packet.message_id, "Receiver".to_string(), PayloadType::Ack);
    ...
    loop {
        ...
        if let Ok(encrypted) = session_crypto.encrypt(trimmed.as_bytes()) {
            let packet = Packet::new(msg_id, "Receiver".to_string(), PayloadType::TextChat(encrypted));
